...
// Output associated with an outpoint, and its number of confirmations.
func (client *client) Output(ctx context.Context, outpoint Outpoint) (Output, int64, error) {
	resp := btcjson.TxRawResult{}
	if err := client.send(ctx, &resp, "getrawtransaction", chainhash.Hash(outpoint.Hash).String(), 1); err != nil {
		return Output{}, 0, fmt.Errorf("bad \"gettxout\": %v", err)
	}
	...
	output := Output{
		Outpoint:     outpoint,
		Value:        pack.NewU64(uint64(amount)),
	...
	return output, confirmations, nil
}

// UnspentOutputs spendable by the given address.
func (client *client) UnspentOutputs(ctx context.Context, minConf, maxConf int64, address Address) ([]Output, error) {
	resp := []btcjson.ListUnspentResult{}
	if err := client.send(ctx, &resp, "listunspent", minConf, maxConf, []string{address.EncodeAddress()}); err != nil && err != io.EOF {
...
// Confirmations of a transaction in the Bitcoin network.
func (client *client) Confirmations(ctx context.Context, txHash pack.Bytes32) (int64, error) {
	resp := btcjson.GetTransactionResult{}
	if err := client.send(ctx, &resp, "gettransaction", hex.EncodeToString(txHash.Bytes())); err != nil {
...
func (client *client) SubmitTx(ctx context.Context, tx Tx) (pack.Bytes32, error) {
	...
	if err != nil {
		return pack.Bytes32{}, fmt.Errorf("bad tx: %v", err)
	}
	resp := ""
	if err := client.send(ctx, &resp, "sendrawtransaction", hex.EncodeToString(serial)); err != nil {
		return pack.Bytes32{}, fmt.Errorf("bad \"sendrawtransaction\": %v", err)
	}
	hash := [32]byte{}
	txid, err := hex.DecodeString(resp)
	if err != nil {
		return pack.Bytes32{}, fmt.Errorf("bad txid: %v", err)
	}
	if len(txid) != 32 {
		return pack.Bytes32{}, fmt.Errorf("bad txid: %v", resp)
	}
	copy(hash[:], txid)
